...
#define SPI_wait_while_transmit(void)	 	while (SPI2->SR & SPI_SR_BSY)
#define SPI_wait(void)						for (int i = 0; i < 5000; i++) {}
#define enc28j60_select()	 				GPIOB->ODR &= ~GPIO_ODR_ODR12
#define enc28j60_release()	 			SPI_wait_while_transmit(); GPIOB->ODR |= (GPIO_ODR_ODR12)
