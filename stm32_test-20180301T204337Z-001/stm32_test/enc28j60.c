...
uint8_t enc28j60_rxtx(uint8_t data)
{
	//SPI_TX = data;
	//debug=================================================================
	//if((data & 0xE0) == 0x0 || (data & 0xE0) == 0x20) ReadOperation = 1;
	//else ReadOperation = 0;
	//debug=================================================================
	...
	//SPI_RX = SPI2_Receive();					//debug
	//SPI_wait_while_transmit();				//debug
	//SPI_wait();												//debug
	
	for (int i = 0; i < 1000; i++);			//need to be a littele timing
